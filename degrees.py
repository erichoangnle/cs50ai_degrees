...
def shortest_path(source, target):
    ...
    while True:
        ...
        # If empty frontier, then on path
        if frontier.empty():
            return None
        ...
        # If node contain target, return solution
        ...
        for state in neighbors_for_person(node.state[1]):
            if not frontier.contains_state(state) and state not in explored and state[1] != node.state[1]:
